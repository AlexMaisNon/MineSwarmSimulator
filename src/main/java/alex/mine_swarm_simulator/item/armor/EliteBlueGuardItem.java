...
public class EliteBlueGuardItem extends ModelTrinketItem {
	public EliteBlueGuardItem() {
		super(new Item.Settings(), "right_guard", ArmorModelHandler.ELITE_BLUE_GUARD, "textures/models/armor/elite_blue_guard.png");
	}
	...
	public Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> getModifiers(ItemStack stack, SlotReference slot, LivingEntity entity, Identifier slotIdentifier) {
		...
		modifiers.put(ModAttributes.PLAYER_CAPACITY, new EntityAttributeModifier(
			Identifier.of(MineSwarmSimulator.MOD_ID, "elite_blue_guard.capacity"),
			75000d,
			EntityAttributeModifier.Operation.ADD_VALUE
		));
		modifiers.put(ModAttributes.PLAYER_BLUE_POLLEN, new EntityAttributeModifier(
			Identifier.of(MineSwarmSimulator.MOD_ID, "elite_blue_guard.blue_pollen"),
			0.25d,
			EntityAttributeModifier.Operation.ADD_VALUE
		));
		modifiers.put(ModAttributes.PLAYER_DEFENSE, new EntityAttributeModifier(
			Identifier.of(MineSwarmSimulator.MOD_ID, "elite_blue_guard.defense"),
			0.07d,
			EntityAttributeModifier.Operation.ADD_VALUE
		));
		modifiers.put(ModAttributes.PLAYER_CRITICAL_POWER, new EntityAttributeModifier(
			Identifier.of(MineSwarmSimulator.MOD_ID, "elite_blue_guard.critical_power"),
			0.5d,
			EntityAttributeModifier.Operation.ADD_VALUE
		));
		modifiers.put(ModAttributes.PLAYER_BLUE_BEE_ATTACK, new EntityAttributeModifier(
			Identifier.of(MineSwarmSimulator.MOD_ID, "elite_blue_guard.blue_bee_attack"),
			1d,
			EntityAttributeModifier.Operation.ADD_VALUE
		));
		return modifiers;
