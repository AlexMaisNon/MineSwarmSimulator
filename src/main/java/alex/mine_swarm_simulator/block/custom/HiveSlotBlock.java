...
public class HiveSlotBlock extends BlockWithEntity {
	...
	protected MapCodec<? extends BlockWithEntity> getCodec() {
		return createCodec(HiveSlotBlock::new);
	}
	...
	protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		if(!world.isClient() && world instanceof ServerWorld serverWorld && world.getBlockEntity(pos) instanceof HiveSlotBlockEntity hiveSlotBlockEntity) {
			...
			if(selectedBee >= 0) {
				...
				if (serverWorld.getEntity(hiveSlotBlockEntity.getBeeUUID()) instanceof BeeEntity oldBeeEntity) {
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return validateTicker(type, ModBlockEntities.HIVE_BLOCK_ENTITY, HiveSlotBlockEntity::tick);
	}
}
